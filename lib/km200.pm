...
sub Log3($$$) { return undef; }
my %defs;
sub RemoveInternalTimer($;$) { return undef;  }
sub readingsSingleUpdate($$$$) { return undef; }
sub InternalTimer($$$;$) { return undef; } 
sub HttpUtils_BlockingGet($)
{
	...
	my $err = "";
        ...
        $request->header("User-Agent" => "TeleHeater");

        my $ua = new LWP::UserAgent();
        my $response = $ua->request($request);
	return ($err, $response->decoded_content());
}
...
sub km200_Define($$)
{
	...
	#return undef;
	return $km200_crypt_key_private;
}
